...
// Shared between weekly-announcement-bot and admin-api, so both compute the
// same "next message" content from the same rows. Takes RRule as a
// parameter rather than requiring it directly - Node's module resolution
// for a bare `require('rrule')` from this file would only look in
// shared/node_modules and its ancestors, not in a sibling package's
// node_modules, so each consumer brings its own rrule dependency instead.
...
function createEventsModule(RRule) {
   ...
   *
   * @param {{ dateLabel: string, title: string }[]} entries
   * @returns {string}
   */
  ...
  return { buildEventEntries, formatMessage };
}

module.exports = { createEventsModule, parseDate, formatDateRange, extractByday };
